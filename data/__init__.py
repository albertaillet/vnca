...
def load_data(batch_size: int, dataset: str = 'binarized_mnist', *, key: Array) -> Tuple[Iterator, Iterator]:
    '''Load binarized MNIST dataset.'''
    train_dataset, test_dataset = get_data(dataset, pad=2)

    def dataset_iterator(dataset: Array, batch_size: int, key: Array) -> Iterator:
        n = len(dataset)
        while True:
...
def load_data_on_tpu(devices: list, dataset: str = 'binarized_mnist', *, key: Array) -> Tuple[Array, Array]:
...
def indicies_tpu_iterator(n_tpus: int, batch_size: int, dataset_size: int, gradient_steps: int, key: Array, device_iterations: int):
